...
function Square ({ xPos, isOpen }) {
    ...
    return(
        ...
        animate={{
            ...
            x: isOpen ? (c_shift + 57.2) * xPos : xPos * 57.2
        }}
        transition={{
            duration: duration,
            ease: "easeInOut"
...
function ShiftingC ({ xPos, isOpen }){
    return(
        <motion.path //black C
        className={`color${xPos % 4}`}
        animate={{x: isOpen ? c_shift + (c_shift * xPos) + (57.2 * xPos) : 0 + (57.2 * xPos)}}
        transition={{
            duration: duration,
            ease: "easeInOut"
        }}
...
function Row ({ isMirrored, isOpen, isHovered }){
    ...
    return(
        <motion.g
            ...
            animate={{
                originX: isOpen ? (viewWidthMax / 2) / 100 : (viewWidthMin / 2) / 100, //Sets origin to center
                transition: {
                    duration: duration,
                    ease: "easeInOut"
                },
        ...
        >
            {/* thank you Rob Hess */}
            {Array.from(Array(width)).map((_, xPos) => <Square key={xPos} xPos = {xPos} isOpen={isOpen} />)}
            ...
            {Array.from(Array(width)).map((_, xPos) => <ShiftingC key={xPos} xPos = {xPos} isOpen={isOpen} />)}
        </motion.g>
    )
}
...
export default function NavToggle ({ isOpen, setIsOpen }) {
    ...
    return(
        <div className="navSelectorWrapper">
            <motion.div
            ...
            >
                <motion.svg //New View Box
                    ...
                    animate={{viewBox: isOpen ? `0 0 ${ viewWidthMax + 33.2 } ${67.17}` : `0 0 ${ viewWidthMin + 33.2 } ${67.17}`}}
                    transition={{
                        duration: duration,
                        ease: "easeInOut"
                    }}
                ...
                >
                    ...
                    <Row isMirrored={true} isOpen={isOpen} isHovered={isHovered}/>
